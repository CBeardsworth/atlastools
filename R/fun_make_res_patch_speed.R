...
atl_res_patch_speed <- function(data,
  ...
  assertthat::assert_that(min(c(max_speed, lim_spat_indep,lim_time_indep, min_fixes)) > 0,
  ...
  tryCatch(
    expr = {
      ...
      # nest data
      data <- data[, list(list(.SD)), by = list(id, patch)] # nests data for id and patch
